...
void pidaux ()
{
  ...
  strcpy(path,"/proc/");
  strcat(path,"uptime"); //how long system is on since previous restart
  ...
  while ((entry = readdir (dirp)) != NULL)
  {
	...
	strcat(path,"/status"); //gives stat info in human readable format
	...
	if(fp!=NULL)
	{
		...
		sscanf(line,"VmRSS:     %llu kB",&memory_rss); //rss - Amount of RAM allocated to a process
	...
	strcpy(path,"/proc/");
	strcat(path,entry->d_name);
	strcat(path,"/stat"); //has all ids - pid,gid,etc.------> status info about process
	fp=fopen(path,"r");
	getline(&line,&len,fp);
	char comm[10],state;
	unsigned int flags;
	...
	strcpy (path, "/proc/");
	strcat (path, entry->d_name);
	strcat (path, "/comm"); //file name of the executable
